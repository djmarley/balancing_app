...
def index():
    ...
    gl_data = conn.execute("SELECT * FROM gl_balance WHERE date = ? order by account", (report_date,)).fetchall()
    # Read Query 2 data (EZ Teller Balance) for the selected date
    ez_data = conn.execute("SELECT * FROM ez_teller_balance WHERE date = ? order by account", (report_date,)).fetchall()
    ...
    recorded_actions = conn.execute(
        "SELECT account, user, timestamp, note FROM mismatch_actions WHERE action_date = ?",
        (report_date,)
    ).fetchall()
    ...
    actions = {row["account"]: {"user": row["user"], "timestamp": row["timestamp"], "note": row["note"]} for row in recorded_actions}
...
def record(account):
    ...
    # ✅ Fetch the branch name from the GL Balance table
    branch_query = conn.execute(
        "SELECT branch_name FROM gl_balance WHERE account = ? AND date = ? LIMIT 1",
        (account, report_date)
    ).fetchone()
    
    branch = branch_query["branch_name"] if branch_query else "Unknown Branch"

    user = "test_user123"  # Replace with actual authentication

    if request.method == 'POST':
        note = request.form.get('note', '')

        # ✅ Insert action for the correct date
        conn.execute(
            "INSERT INTO mismatch_actions (account, action_date, note, user) VALUES (?, ?, ?, ?)",
            (account, report_date, note, user)
        )
        conn.commit()
        conn.close()
        flash(f"Action recorded for account {account} on {report_date}.", "success")
        return redirect(url_for('index', report_date=report_date))

    else:
        # ✅ Retrieve existing note for this account & date
        action = conn.execute(
        ...
        return render_template('record.html', account=account, branch=branch, report_date=report_date, note=note)
